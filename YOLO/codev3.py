...
# Variável para armazenar a posição anterior
previous_positions = {}
...
def draw_direction(canvas, positions):
    ...
    for center_x, center_y in positions:
        ...
        # Verifica se a posição anterior existe para o objeto detectado
        closest_key = None
        min_distance = float('inf')
        
        # Encontra o objeto anterior mais próximo com base em distância Euclidiana
        for prev_key in previous_positions:
            prev_x, prev_y = previous_positions[prev_key]
            distance = np.sqrt((center_x - prev_x) ** 2 + (adjusted_center_y - prev_y) ** 2)
            if distance < min_distance:
                min_distance = distance
                closest_key = prev_key
        
        # Se a distância for razoável (por exemplo, menor que 50 pixels), desenha a direção
        if closest_key and min_distance < 50:
            prev_x, prev_y = previous_positions[closest_key]
            # Desenha a linha de direção
            canvas.create_line(prev_x, prev_y, center_x, adjusted_center_y, fill="blue", width=5)
        ...
        previous_positions[(center_x, adjusted_center_y)] = (center_x, adjusted_center_y)
