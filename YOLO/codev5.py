...
TIME_BETWEEN_FRAMES = 0.05  # segundos
...
def draw_direction(canvas, positions):
    ...
    for center_x, center_y, label in positions:
        ...
        # Estimativa da próxima posição (baseada na velocidade)
        if len(object_positions[label]) > 1:
            ...
            # Calcula a diferença (deslocamento)
            delta_x = last_x - prev_x
            delta_y = last_y - prev_y

            # Estima a próxima posição (projeção)
            predicted_x = last_x + delta_x
            predicted_y = last_y + delta_y

            # Calcula a velocidade do objeto
            speed = calculate_speed(prev_x, prev_y, last_x, last_y)

            # Desenha a estimativa da próxima posição
            canvas.create_oval(predicted_x - radius, predicted_y - radius, predicted_x + radius, predicted_y + radius, outline="yellow", width=2)
